function list_clicked(id, index, old_index) {
	if( $(".international").hasClass("selected") ) {
		$("#input-beneficiary-account").removeClass("format-iban");
		var tmp = $("#input-beneficiary-account").val();
		$("#input-beneficiary-account").val(tmp.replace(/ /g, ""));
	}else {
		$("#input-beneficiary-account").addClass("format-iban");
		var tmp = $("#input-beneficiary-account").val();
		$("#input-beneficiary-account").val(split_iban(tmp));
	}
}
...
$(document).ready(function() {
	...
	$("#filters-trigger").click(function(){
		if($(this).hasClass("open")){
			...
			$(this).text(error_messages["showFilters"]);
		}else{
			$(this).parent().parent().next().slideDown();
			$(this).addClass("open");
			$(this).text(error_messages["hideFilters"]);
			scroll.list(300);
		}
	});
	...
	$(".parties").each(function(){
		$(this).children("li:not(.non-selectable):first").addClass("selected");
	});
	...
	$("#input-beneficiary-account").blur(function(){
		...
		submit_status = true;//GIV:BUG_1064_UIF 20120326
...
//GIV:BUG_1064_UIF 20120326 Online activity -beneficiary account filter:START:			
			var transactionTypeDescr = "";
			
			if(transaction_type == "0111" || transaction_type == "0113" || transaction_type == "0114")
				transactionTypeDescr = "domestic";
			else if(transaction_type == "0003" || transaction_type == "0012" || transaction_type == "0013" ||
					transaction_type == "0014" || transaction_type == "0110" || transaction_type == "0009" ||
					transaction_type == "0019" || transaction_type == "0022" || transaction_type == "0027")
				transactionTypeDescr = "intrabank";
			//THERE IS CURRENTLY NO VALID international type validation either JS or server side
//			else if(transaction_type == "0137" || transaction_type == "0138" || transaction_type == "0139" || transaction_type == "0140") 
//				transactionTypeDescr = "international";
				
			if(transactionTypeDescr != ""){
				show_loading(false);
				setTimeout(function(){check_iban("input-beneficiary-account", beneficiary_account, transactionTypeDescr)}, 50);
				setTimeout(function(){remove_loading()},100);
/*				var validationResult = beneficiary_server_validation(null, beneficiary_account, transactionTypeDescr);
				if( validationResult.isValid == '1'){
					submit_status = true;
				}else if( validationResult.isValid == '0'){
					submit_status = false;
...
/*
			if(transaction_type == "0111" || transaction_type == "0113" || transaction_type == "0114"){
				...
				if(status == true){
					...
					$.post('AJAXServlet',
						{
						 operation : 			'isAccountFormatValidFor0111',
...
*/
//GIV:BUG_1064_UIF 20120326 Online activity -beneficiary account filter:END
		}
	});
	...
	$("form").submit(function(){
		...
		if(!$("#input-beneficiary-account").is(":visible")){
			...
			$("#input-beneficiary-account").blur();
		}
...
//Custom methods
	function crButton_onclick(href,idx){
		var entryPoint = 'AJAXServlet';
		var operation = 'sessionSummaryGetCyberReceipt';
