...
$(document).ready(function(){
	...
	$(".reset").click(function(){
		$(".fail, .success").not("#start-date").remove();
	...
	var account_validations = {
		...
		ownership_validation: function(){
			...
			if(fromAccount != "" && toAccount != ""){
				...
				$.post(
					...
					{
						<%=WebParams.ajaxServletMethod.id()%>:		'<%=AJAXServlet.Operations.checkAccountsHaveSameOwnershipFor0237%>',
						<%=WebParams.conversationId.id()%>:			$('input[name=<%=WebParams.conversationId.id()%>]').val(),
						<%=WebParams.fromAccount.id()%>:				fromAccount,
						<%=WebParams.toAccount.id()%>:				toAccount
					},
		...
		display_error: function(jsArray){
			...
			if(jsArray.hasErrorsTextFlag == "1"){
...
//		var input = parseFloat(amount_to_number($("#transfer-amount").val().replace(",", "")));
...
console.log("max_amount="+max_amount);
	...
	$("form").submit(function(){
		...
		if(max_amount != 0) {
			var validation = [[$("#transfer-amount"), "max", max_amount], "compare_numbers", "empty_string"];
	...
	/*	//kyrD:20120206:Usability:transferFromTimeAccount
		This validation must be done only each time the user selects another account.
		If we leave this validation then the function ownership_validation makes an asynchronous call and the value
		submit_status has the value undefined.
		if(submit_status == true){
